// [協作區] SkateTrackPackageSnowCompatibilityTests.swift
import Foundation
import XCTest
...
final class SkateTrackPackageSnowCompatibilityTests: XCTestCase {
    ...
    func testFormatDiagnosticsAndSnowPayloadCapabilitiesRemainDisjoint() throws {
        ...
        let data = try SkateTrackPackageWriter().encodedData(for: package)
        let decoded = try SkateTrackPackageReader().decodePackage(from: data)
        let decodedFormatCapabilities = Set(try XCTUnwrap(decoded.manifest.formatCapabilities))
        let decodedPayloadCapabilities = Set(try XCTUnwrap(decoded.manifest.capabilities))

        XCTAssertEqual(decodedFormatCapabilities, Set(formatCapabilities))
        XCTAssertEqual(decodedPayloadCapabilities, Set(SkateTrackPackageSnowCapability.allRawValues))
        XCTAssertTrue(decodedFormatCapabilities.isDisjoint(with: decodedPayloadCapabilities))
        XCTAssertTrue(decoded.manifest.declaresSnowPackagePayload)
    }
