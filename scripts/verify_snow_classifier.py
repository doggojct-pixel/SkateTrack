...
def verify_a005_motion_sample_diff() -> None:
    rel_path = "Shared/Models/MotionSample.swift"
    result = subprocess.run(
        ["git", "-C", str(ROOT), "diff", "--unified=0", "HEAD", "--", rel_path],
        check=False,
        capture_output=True,
        text=True,
    )
    if result.returncode != 0:
        fail("unable to inspect MotionSample.swift diff against HEAD")
    added = [
        line[1:].strip()
    ...
    expected_added = ["case (.snow, _):", "return .snowReserved"]
    if added != expected_added or removed:
        fail(
            "A005 MotionSample compatibility diff must contain only the explicit "
            f"Snow fidelity mapping; added={added}, removed={removed}"
        )
...
def read_head(rel_path: str) -> str:
    ...
    if result.returncode != 0:
        fail(f"unable to read HEAD content for {rel_path}")
...
def verify_no_scope_creep() -> None:
    ...
    for path in ROOT.glob("**/*.swift"):
        ...
        if "Shared/WatchBridge/" in rel and (not A005_INTEGRATION_MODE or rel in changed):
...
def verify_existing_value_types_not_expanded() -> None:
    if A005_INTEGRATION_MODE:
        verify_a005_motion_sample_diff()
    for rel_path in UNMODIFIED_MODEL_FILES:
        text = read(rel_path)
        head_text = read_head(rel_path) if A005_INTEGRATION_MODE and rel_path == "Shared/Models/MotionSample.swift" else ""
        ...
        for token in FORBIDDEN_MODEL_TOKENS.get(rel_path, []):
            if token in text and (not head_text or token not in head_text):
