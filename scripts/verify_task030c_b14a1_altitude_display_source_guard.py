#!/usr/bin/env python3
"""Verify Task-030c-b15-B-3 altitude chart source guard remains display-only."""
...
REQUIRED = {
    ...
    "scripts/verify_task030c_b14a1_altitude_display_source_guard.py": [
        "Task-030c-b15-B-3 altitude chart source guard remains display-only",
    ],
...
def main() -> int:
    ...
    if failures:
        print("Task-030c-b15-B-3 altitude chart source guard check failed:", file=sys.stderr)
        for failure in failures:
            print(f"  {failure}", file=sys.stderr)
        return 1
    print("Task-030c-b15-B-3 altitude chart source guard checks passed.")
    return 0
