#!/usr/bin/env python3
"""Verify Task-030c-b14-B-1 startup route visual suppression remains display-only."""
...
REQUIRED = {
    ...
    "scripts/verify_task030c_b14a2_startup_route_visual_suppression.py": [
        "Task-030c-b14-B-1 startup route visual suppression remains display-only",
    ],
...
def main() -> int:
    ...
    if failures:
        print("Task-030c-b14-B-1 startup route visual suppression check failed:", file=sys.stderr)
        for failure in failures:
            print(f"  {failure}", file=sys.stderr)
        return 1
    print("Task-030c-b14-B-1 startup route visual suppression checks passed.")
    return 0
