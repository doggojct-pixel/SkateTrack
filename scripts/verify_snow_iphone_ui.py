...
def main() -> None:
    ...
    require("SnowDistanceInspectorView(breakdown: snowSession.state.distanceBreakdown)" in session_summary_view, "SessionSummaryView must show SnowDistanceInspectorView for snow sessions")
    require("SnowSegmentTimelineView(segments: snowSession.state.segments)" in session_summary_view, "SessionSummaryView must show SnowSegmentTimelineView for snow sessions")
    ...
    print("[snow-task-005] PASS: iPhone Snow live data boundary, session lifecycle wiring, SnowHUDView four-state UI, repository-backed Snow summary/timeline/inspector, config-driven lowConfidence policy, mapper tests, and scope guardrails are present.")
