...
REQUIRED_SNIPPETS = {
    ...
    "Tests/iOSTests/SnowSessionRepositoryTests.swift": [
        ...
        "migratedMetadata[NSStoreUUIDKey]",
        "PersistedSnowRun",
        "PersistedSnowSegment",
        "snowRepository.fetchRun",
    ],
...
def verify_no_prototype_swift() -> None:
    ...
    for path in ROOT.glob("**/*.swift"):
        path_text = rel(path)
        if any(part in path_text for part in FORBIDDEN_PATH_PARTS) and (
            not A005_INTEGRATION_MODE or path_text in changed
        ):
