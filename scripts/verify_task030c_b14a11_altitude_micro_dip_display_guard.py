#!/usr/bin/env python3
"""Verify Task-030c-b14-B-1 altitude micro-dip display guard remains display-only."""
...
REQUIRED = {
    ...
    "scripts/verify_task030c_b14a11_altitude_micro_dip_display_guard.py": [
        "Task-030c-b14-B-1 altitude micro-dip display guard remains display-only",
    ],
...
def main() -> int:
    ...
    if failures:
        print("Task-030c-b14-B-1 altitude micro-dip display guard check failed:", file=sys.stderr)
        for failure in failures:
            print(f"  {failure}", file=sys.stderr)
        return 1
    print("Task-030c-b14-B-1 altitude micro-dip display guard checks passed.")
    return 0
