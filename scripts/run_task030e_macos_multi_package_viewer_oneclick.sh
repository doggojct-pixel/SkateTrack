...

{
  ...
  echo "對齊子任務: task-030e-MacViewer-014 — Final Merge Gate"
  echo "已完成前置: task-030e-MacViewer-013 — Manual QA Gate"
  echo "未實作: Task-031-prep shared activity visualization pipeline, package merge, duplicate deletion, winner selection, local-history import, route correction, road matching, snap-to-road, route reconstruction, location permission, user-location display, route geometry mutation, trusted metrics mutation, package schema change, Core Data write"
  echo "REPO=${REPO}"
  ...
  if [ ! -d "${REPO}/.git" ]; then
    ...
    run_python_verifier "scripts/verify_task030e_final_merge_gate.py"
