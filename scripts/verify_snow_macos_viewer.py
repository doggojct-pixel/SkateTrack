...
def main() -> None:
    ...
    if A005_INTEGRATION_MODE:
        if any(path.startswith("Shared/WatchBridge/") for path in changed):
            fail("A005 integration must not modify Shared/WatchBridge before A006")
        if any(path.endswith(".skatetrack") for path in changed):
            fail("A005 integration must not add binary package samples")
