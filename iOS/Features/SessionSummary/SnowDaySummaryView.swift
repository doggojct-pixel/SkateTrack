...
struct SnowDaySummaryView: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 14) {
            ...
            case let .error(errorKey):
                errorState(errorKey)
            default:
                switch SnowDaySummaryPresentation.resolve(for: state) {
                case .empty:
                    emptyState
                case .unknownOnly:
                    unknownOnlyStatus
    ...
    private var unknownOnlyStatus: some View {
        HStack(alignment: .top, spacing: 10) {
            ...
            VStack(alignment: .leading, spacing: 5) {
                Text("snow.summary.unknownOnly.title")
                    .font(.system(size: 15, weight: .black, design: .rounded))
                    .foregroundStyle(.white)

                Text("snow.summary.unknownOnly.detail")
                    .font(.system(size: 12, weight: .semibold, design: .rounded))
                    .foregroundStyle(SkateTrackSessionStartColors.textSecondary)
                    .fixedSize(horizontal: false, vertical: true)
            }
        ...
        .overlay(RoundedRectangle(cornerRadius: 18).stroke(SkateTrackSessionStartColors.purple.opacity(0.20), lineWidth: 1))
...
enum SnowDaySummaryPresentation: Equatable {
    ...
    static func resolve(for state: SnowSessionState) -> SnowDaySummaryPresentation {
